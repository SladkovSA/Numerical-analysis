...
def real(xt, tt):
    return 3 / 2 * xt ** 2 + np.exp(-tt) * np.arccos(xt / 2)


def f(xt, tt):
    return - 3 / 2 + (xt * np.exp(- tt)) / (2 * (4 - xt ** 2) ** (3 / 2)) + np.exp(- tt) * np.arccos(xt / 2)


def fi(xt):
    return 3 / 2 * xt ** 2 + np.arccos(xt / 2)


def psi(xt):
    return - np.arccos(xt / 2)


def g0(tt):
    return (np.pi - 1 / 2) * np.exp(- tt)


def gl(tt):
    return 3 / 2 + np.pi / 3 * np.exp(- tt)
